...
## 联系方式管理
- **邮箱**: 采用防爬虫格式 `xxx#domain.edu.cn`（#需替换为@），位于Footer组件
- **微信公众号**: 二维码位于Footer组件，支持悬停显示
  - `CVEO课题组`: `/assets/social/wechat-cveo.png`
  - `珞珈经纬`: `/assets/social/wechat-luojiagingwei.png`
- **GitHub**: 外部链接卡片，点击跳转GitHub仓库
- **布局**: Footer采用4个等高卡片布局（GitHub、邮箱、CVEO微信、珞珈微信）
- **防爬虫**: 邮箱地址不转换为`mailto:`链接，防止爬虫收集
...
- 每篇论文一个 Markdown 文件，使用 Frontmatter 结构化字段：
  ...
  - `partition`: 中科院分区（可选，枚举值：`中科院一区`、`中科院二区`、`中科院三区`、`中科院四区`）
  - `ccfPartition`: CCF分区（可选，枚举值：`CCF-A`、`CCF-B`、`CCF-C`）
...
- **分区说明**：
  - `partition`: 适用于期刊论文，按影响因子分为四个区
  - `ccfPartition`: 适用于会议和期刊论文，中国计算机学会推荐目录，分为A/B/C三级
  - 两个字段可同时存在（如某期刊既是中科院一区，又是CCF-A）
- **重要说明**：
...
## 合作单位
- 文件位置：`src/data/partners.json`
- 数据结构：
```json
{
  "name": "单位名称",
  "logo": "/assets/partners/example.png",  // 可选
...
- **字段说明**：
  - `name`: 单位名称（必填，字符串）
  - `logo`: 单位logo路径（可选，字符串）
    - 存储位置：`public/assets/partners/`
    - 支持方形或横向logo，建议高度统一
  - `website`: 官方网站链接（可选，字符串）
  - `description`: 单位简短描述（可选，字符串）

## 科研合作平台
- 文件位置：`src/data/research-centers.json`
- 数据结构：
```json
{
  "name": "平台名称",
  "partners": ["合作单位1", "合作单位2"],
  "type": "科研合作平台",
  "established": "2023",
  "description": "详细描述",
  "shortDescription": "简短描述",  // 用于卡片展示
  "image": "/assets/research-centers/example.jpg",  // 4:3牌匾图片
  "focusAreas": ["研究方向1", "研究方向2"]  // 可选
}
```
- **字段说明**：
  - `name`: 平台名称（必填，字符串）
  - `partners`: 合作单位列表（必填，字符串数组）
  - `type`: 平台类型（必填，字符串）
  - `established`: 成立年份（必填，字符串）
  - `description`: 详细描述（必填，字符串）
  - `shortDescription`: 简短描述（可选，字符串），用于卡片展示
  - `image`: 4:3牌匾图片路径（可选，字符串）
    - 存储位置：`public/assets/research-centers/`
    - 建议比例：4:3
  - `focusAreas`: 研究方向列表（可选，字符串数组）

## 团队介绍
- 文件位置：`src/data/about.json`
- 包含团队基本信息、研究方向、统计数据、合作卡片等

## 校友数据
- 文件位置：`src/data/alumni.json`（博士/硕士校友）
- 文件位置：`src/data/undergrad-alumni.json`（本科校友）
- 包含校友姓名、毕业年份、去向等信息
