...
- **合作与平台**: `src/pages/collaboration.astro`
...
- **组件分类**:
  ...
  - 内容展示组件：`PaperCard.astro`, `MemberCard.astro`, `ProjectCard.astro`, `NewsCard.astro`
  - 数据表格组件：`PatentTable/`, `InventionTable.astro`, `SoftwareTable.astro`, `AlumniTable.astro`
  - 合作展示组件：`CollaborationEntryCards.astro`, `ResearchCenterList.astro`, `PartnerGrid.astro`
  - 特殊组件：`TimeAxis.astro`
- **开发规范**:
  - 组件 props 必须有明确的接口定义
  - 使用 Astro 组件最佳实践
...
- **数据工具**: `src/lib/data/` - 数据加载和处理的工具函数
  ...
  - `news.ts` - 新闻数据相关函数
- **直接导入**: 合作单位、科研合作平台、团队介绍等数据直接导入JSON文件
  ```typescript
