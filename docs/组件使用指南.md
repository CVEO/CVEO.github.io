...
### 合作入口卡片 (CollaborationEntryCards)
```astro
---
import CollaborationEntryCards from '../components/CollaborationEntryCards.astro'
import aboutData from '../data/about.json'
---

<CollaborationEntryCards cards={aboutData.collaborationCards} />
```

### 科研合作平台列表 (ResearchCenterList)
```astro
---
import ResearchCenterList from '../components/ResearchCenterList.astro'
import researchCentersData from '../data/research-centers.json'
---

<ResearchCenterList centers={researchCentersData} />
```
- **特点**：
  - 紧凑网格布局（桌面端2列，移动端1列）
  - 支持4:3牌匾图片展示
...
<PartnerGrid partners={partnersData} />
```
- **特点**：
  - 高密度网格布局（最多6列）
  - 自适应logo显示（支持方形/横向）
...
```astro
---
import AlumniTable from '../components/AlumniTable.astro'
import alumniData from '../data/alumni.json'
---

<AlumniTable alumni={alumniData} />
```

### 页脚组件 (Footer)
- **自动包含**：所有页面通过BaseLayout自动包含Footer
- **功能**：
  - 4个等高卡片布局（GitHub、邮箱、CVEO微信、珞珈微信）
  - 微信公众号二维码悬停显示
  - 邮箱防爬虫格式
  - 版权信息和导航链接
- **自定义**：直接编辑 `src/components/Footer.astro`
